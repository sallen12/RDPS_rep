lspm_fa <- function(X, yvect, xnew, ysets) {
  if (length(xnew) > 1) {
    lapply(seq_along(xnew), function(i) {print(i); lspm_fa(X, yvect, xnew[i], ysets)})
  } else {
...
krrpm_fa <- function(X, yvect, xnew, ysets, a = 0) {
  if (length(xnew) > 1) {
    lapply(seq_along(xnew), function(i) {print(i); krrpm_fa(X, yvect, xnew[i], ysets, a)})
  } else {
    ...
    K <- exp(-abs(outer(Xf, Xf, "-"))) # uses laplacian kernel
    H <- K %*% solve(K + a*diag(n + 1))
